...
usl_windmax <- read.delim(
  "https://dl.dropboxusercontent.com/s/8s6kzrkglrrhtvi/USL_MET_IP_WindMax.dat?dl=0",
  header = T, sep=",", skip = 3
)
...
#Removing this section for 2022 becaue we don't plan on swapping, unhash to active
...
#Filter 2022 data
usl <- filter(usl, date.time>=as_datetime("2022-04-16"))
usl_met_avg <- filter(usl_met_avg, date.time>=as_datetime("2022-04-16"))
usl_rain <- filter(usl_rain, date.time>=as_datetime("2022-04-16"))
usl_wind <- filter(usl_wind, date.time>=as_datetime("2022-04-16"))
usl_windmax <- filter(usl_windmax, date.time>=as_datetime("2022-04-16"))
...
for(n in 1:length(usl.days)){
  days = which(as_datetime(usl$date.time) >= as_datetime(as.Date(usl.days[n])) & as_datetime(usl$date.time) < as_datetime(as.Date(usl.days[n])+1))
  for(i in unique(usl$profile[days])){
    profile = which(usl$profile[days] %in% i)
    usl$date.time[days[profile]] = usl$date.time[days[profile[1]]]
  }
}
...
write.csv(usl, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl.csv", row.names = FALSE)
write.csv(usl_met_avg, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl_met_avg.csv", row.names = FALSE)
write.csv(usl_rain, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl_rain.csv", row.names = FALSE)
write.csv(usl_surf, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl_surf.csv", row.names = FALSE)
write.csv(usl_wind, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl_wind.csv", row.names = FALSE)
write.csv(usl_windmax, "C:/Users/admin/Dropbox (ADK Watershed)/ADK Watershed Team Folder/Env Research Lab/USL_Profiler_Data/Web_Files/Cleaned Data/usl_windmax.csv", row.names = FALSE)

#Harmonize data between deployments
#for(i in 1:length(usl_sonde_swap$date.time)){
#  usl.before <- filter(usl, as_datetime(date.time)<=as_datetime(usl_sonde_swap$date.time[i]))
#  usl.profile.before <- filter(usl.before, date.time == max(usl.before$date.time))
  
#  usl.after <- filter(usl, as_datetime(date.time)>=as_datetime(usl_sonde_swap$date.time[i]))
...
#  usl$cond[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(cond.fit, newdata = data.frame("After" = usl.after$cond))
...
#  usl$ODO.percent[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(ODO.percent.fit, newdata = data.frame("After" = usl.after$ODO.percent))
...
#  usl$ODO[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(ODO.fit, newdata = data.frame("After" = usl.after$ODO))
...
#  usl$pH[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(pH.fit, newdata = data.frame("After" = usl.after$pH))
...
#  usl$turbidity[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(turbidity.fit, newdata = data.frame("After" = usl.after$turbidity))
...
#  usl$bga.pc.rfu[as_datetime(usl$date.time) >= as_datetime(usl_sonde_swap$date.time[i])] = predict(bga.pc.rfu.fit, newdata = data.frame("After" = usl.after$bga.pc.rfu))
